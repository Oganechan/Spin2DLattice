...
class Geometry {
  public:
    ...
    // Returns the linear size of the system (number of unit cells along each
    // dimension)
    ...
    inline const std::vector<std::array<int32_t, 3>> &get_atom_indices() const {
    ...
    inline const std::vector<std::vector<std::vector<int32_t>>> &
    get_neighbor_table() const {
    ...
    inline int32_t get_atom_id(const int32_t cell_i, const int32_t cell_j,
                               const int32_t atom_in_cell_id) const {
        int32_t taco_i = (cell_i % system_size_ + system_size_) % system_size_;
        int32_t taco_j = (cell_j % system_size_ + system_size_) % system_size_;

        return (taco_i + taco_j * system_size_) * basis_count_ +
               atom_in_cell_id;
    }
    ...
    inline std::array<double, 2> get_atom_position(int32_t atom_id) const {
        ...
        double x = cell_i * lattice_vectors_[0][0] +
                   cell_j * lattice_vectors_[1][0] +
                   basis_vectors_[atom_in_cell_id][0];

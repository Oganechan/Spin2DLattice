...
double lattice::Geometry::get_distance(int32_t first_atom_id, int32_t second_atom_id) const
{
    ...
    const double delta_x = atom_positions_[second_atom_id][0] - atom_positions_[first_atom_id][0];
    const double delta_y = atom_positions_[second_atom_id][1] - atom_positions_[first_atom_id][1];
    ...
    if (boundary_type_ == BoundaryType::PERIODIC)
    {
        ...
        for (int32_t n = -1; n <= 1; ++n)
        {
            ...
            {
                const double taco_dx = (n * lattice_vectors_[0][0] + m * lattice_vectors_[1][0]) * system_size_ + delta_x;
                const double taco_dy = (n * lattice_vectors_[0][1] + m * lattice_vectors_[1][1]) * system_size_ + delta_y;
...
void lattice::Geometry::initialize_lattice()
{
    ...
    case CrystalType::C_RECTANGULAR:
        ...
        basis_vectors_ = {
            {0.0, 0.0},
            {0.5 * lattice_constant_a_, 0.5 * lattice_constant_b_}};
        lattice_vectors_ = {
            {lattice_constant_a_, 0.0},
            {0.0, lattice_constant_b_}};
        break;
...
{
    if (system_size_ <= 0)
        throw std::invalid_argument("Linear size must be positive, got: " + std::to_string(system_size_));

    if (shell_count_ <= 0)
        throw std::invalid_argument("Number of shells must be positive, got: " + std::to_string(shell_count_));

    if (lattice_constant_a_ <= 0 || lattice_constant_b_ <= 0)
        throw std::invalid_argument("Lattice constants must be positive, got norm_a: " +
                                    std::to_string(lattice_constant_a_) + ", norm_b: " + std::to_string(lattice_constant_b_));

    if (lattice_vectors_.empty() || lattice_vectors_.size() != 2)
        throw std::runtime_error("Translation vectors not properly initialized");

    const double total_system_x = (std::abs(lattice_vectors_[0][0]) + std::abs(lattice_vectors_[1][0])) * system_size_;
    const double total_system_y = (std::abs(lattice_vectors_[0][1]) + std::abs(lattice_vectors_[1][1])) * system_size_;

    const double max_system_distance = (boundary_type_ == BoundaryType::PERIODIC)
...
void lattice::Geometry::compute_neighbors()
{
    ...
    for (int32_t atom_id = 0; atom_id < atom_count_; ++atom_id)
    {
        ...
        {
            for (int32_t new_atom_in_cell_id = 0; new_atom_in_cell_id < basis_count_; ++new_atom_in_cell_id)
            {
                int32_t neighbor_id = get_atom_id(cell_i + di, cell_j + dj, new_atom_in_cell_id);

                if (neighbor_id < 0 || neighbor_id >= atom_count_ || neighbor_id == atom_id)
                    continue;

                if (boundary_type_ == BoundaryType::HARD)
                {
                    auto [ni, nj, npos] = atom_indices_[neighbor_id];
                    if (ni < 0 || ni >= system_size_ || nj < 0 || nj >= system_size_)
                        continue;
                }

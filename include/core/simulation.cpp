...
Simulation::Simulation(const Config &config, const std::string &output_directory)
    : output_directory_(output_directory),
      atoms_(config),
      calculator_(atoms_),
      metropolis_(atoms_),
      data_(output_directory),
      equilibration_sweeps_(config.get<int32_t>("simulation.equilibration_sweeps")),
      production_sweeps_(config.get<int32_t>("simulation.production_sweeps")),
      measurement_interval_(config.get<int32_t>("simulation.measurement_interval")) {}
...
void Simulation::run()
{
    ...
    std::cout << "Execution time: " << std::chrono::duration<double>(end_time - start_time).count() << std::endl;
}

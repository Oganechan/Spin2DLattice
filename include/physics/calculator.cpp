...
double physics::Calculator::calculate_flip_energy_difference(
    ...
    double energy_diff =
        -calculate_spin_dot_product(spin_diff, external_magnetic_field_);

    for (int32_t shell_index = 0; shell_index < shell_count_; ++shell_index) {
        double J = get_exchange_constant(shell_index);
        for (int32_t neighbor_id : neighbor_table_[atom_id][shell_index])
            if (atoms_.get_magnetic_state(neighbor_id))
                energy_diff -= J * calculate_spin_dot_product(
                                       spin_diff, atoms_.get_spin(neighbor_id));
    }
...
double physics::Calculator::calculate_total_magnetization() const {
    ...
    double magnitude =
        std::sqrt(total_magnetization[0] * total_magnetization[0] +

...
TEST_CASE("Test config", "[config]") {
    ...
    auto create_test_config = [](const std::string &path) {
        json test_config = {{"lattice",
                            ...
                            {"physical",
                             {{"exchange_constants", {1.0}},
                              {"external_magnetic_field", {0.0, 0.0, 0.0}},
                              {"temperature", 10.0}}}};
    ...
    SECTION("Loading configuration") {
        ...
        REQUIRE(config.get<std::array<double, 3>>(
    ...
    SECTION("get/set methods") {
        ...
        REQUIRE(config.get<std::string>("test.string_value") ==
                TEST_STRING_VALUE);
        REQUIRE(config.get<double>("test.TEST_ZERO_VALUE") == TEST_ZERO_VALUE);
        REQUIRE(config.get<std::string>("TEST_EMPTY_STRING") ==
                TEST_EMPTY_STRING);
    }
    ...
    SECTION("Non-existent keys, default values") {
        ...
        REQUIRE(config.get<int32_t>("test.non_exist", DEFAULT_INT) ==
                DEFAULT_INT);
        REQUIRE(config.get<double>("test.non_exist_double", DEFAULT_DOUBLE) ==
                DEFAULT_DOUBLE);
        REQUIRE(config.get<std::string>("test.non_exist_string",
    ...
    SECTION("Error handling") {
        ...
        REQUIRE_THROWS_AS(config.load("outside_of_being.json"),
                          std::runtime_error);

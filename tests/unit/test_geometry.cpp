...
TEST_CASE("Test geometry", "[geometry]")
{
    ...
    SECTION("Correct config parsing")
    {
        ...
        config.set<std::vector<double>>("physical.exchange_constants", {1.0, 0.25});
        ...
        REQUIRE(geometry.get_crystal_type() == lattice::CrystalType::RECTANGULAR);
        REQUIRE(geometry.get_boundary_type() == lattice::BoundaryType::PERIODIC);
    ...
    SECTION("Calculating distances within hard boundaries")
    {
        ...
        REQUIRE(geometry.get_distance(4, 1) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 3) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 5) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 7) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));

        REQUIRE(geometry.get_distance(0, 1) == Catch::Approx(geometry.get_distance(1, 0)).margin(1e-10));

        const double DIAGONAL_DISTANCE = std::sqrt(2.0);
        REQUIRE(geometry.get_distance(4, 0) == Catch::Approx(DIAGONAL_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 2) == Catch::Approx(DIAGONAL_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 6) == Catch::Approx(DIAGONAL_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(4, 8) == Catch::Approx(DIAGONAL_DISTANCE).margin(1e-10));
    ...
    SECTION("Calculating distances within periodic boundaries")
    {
        ...
        const double EXPECTED_DISTANCE = 1.0;
        REQUIRE(geometry.get_distance(0, 2) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));
        REQUIRE(geometry.get_distance(0, 6) == Catch::Approx(EXPECTED_DISTANCE).margin(1e-10));

        const double DIAGONAL_DISTANCE = std::sqrt(2.0);
        REQUIRE(geometry.get_distance(0, 8) == Catch::Approx(DIAGONAL_DISTANCE).margin(1e-10));
    }
    ...
    SECTION("Neighbor table for different types of lattice")
    {
        ...
        config.set<std::string>("lattice.crystal_type", "rectangular");
        lattice::Geometry rectangular_geometry(config);
        const auto &rectangular_neighbors = rectangular_geometry.get_neighbor_table();
        ...
        config.set<std::string>("lattice.crystal_type", "honeycomb");
        lattice::Geometry honeycomb_geometry(config);
        const auto &honeycomb_neighbors = honeycomb_geometry.get_neighbor_table();
        for (int32_t atom_id = 0; atom_id < honeycomb_geometry.get_atom_count(); ++atom_id)
        {
            const auto &atom_neighbors = honeycomb_neighbors[atom_id];
            REQUIRE(atom_neighbors.size() == honeycomb_geometry.get_shell_count());

            for (int32_t shell = 0; shell < honeycomb_geometry.get_shell_count(); ++shell)
        ...
        for (int32_t atom_id = 0; atom_id < honeycomb_geometry.get_atom_count(); ++atom_id)
        {
            auto [cell_i, cell_j, atom_in_cell_id] = honeycomb_geometry.get_cell_index(atom_id);
            int32_t reconstructed_id = honeycomb_geometry.get_atom_id(cell_i, cell_j, atom_in_cell_id);
        ...
        for (int32_t atom_id = 0; atom_id < honeycomb_geometry.get_atom_count(); ++atom_id)
        {
            auto computed_position = honeycomb_geometry.get_atom_position(atom_id);
            REQUIRE(computed_position[0] == Catch::Approx(positions[atom_id][0]).margin(1e-10));
            REQUIRE(computed_position[1] == Catch::Approx(positions[atom_id][1]).margin(1e-10));
        }
    }
}